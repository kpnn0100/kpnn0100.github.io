          ...
          >
            {programmingItems.map((project, index) => (
              <motion.article
                ...
                whileHover={{ x: 4, borderColor: "rgba(20,184,166,0.4)" }}
                transition={{ type: "spring", stiffness: 300, damping: 28 }}
                className="border border-white/10 rounded-lg p-8 bg-black/40 backdrop-blur-sm transition-colors duration-300 group"
              >
                {/* Index */}
                <motion.div
                ...
                {/* Title */}
                <h2 className="text-2xl mb-3 tracking-wide group-hover:text-teal-300 transition-colors duration-300">
                  {project.title}
                </h2>
                ...
                {/* Authors */}
                <p className="text-sm text-white/60 mb-4">{project.authors.join(", ")}</p>

                {/* Abstract */}
                <p className="text-white/80 leading-relaxed mb-6">{project.abstract}</p>
                ...
                >
                  {project.tags.map((tag) => (
                    <motion.span
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-teal-500/10 border border-teal-500/20 text-xs text-teal-400 cursor-default"
                    >
